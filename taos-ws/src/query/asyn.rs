...
use ws_tool::frame::SimplifiedHeader;
    ...
    async fn send_recv(&self, msg: WsSend) -> RawResult<WsRecvData> {
        ...
        match msg {
            ...
            WsSend::Binary(bytes) => {
                timeout(
                    SEND_TIMEOUT,
                    self.sender.send_async(WsMessage {
                        code: OpCode::Binary,
                        data: bytes,
                        close_code: None,
                    }),
...
async fn read_queries(
    ...
    let parse_frame = |frame: (SimplifiedHeader, &[u8])| {
...
impl WsTaos {
    ...
    pub(crate) async fn from_wsinfo(info: &TaosBuilder) -> RawResult<Self> {
        ...
        sender
            .send(OpCode::Text, &sonic_rs::to_vec(&version).unwrap())
            .await
            .map_err(Error::from)?;
        sender.flush().await.map_err(Error::from)?;

        let duration = Duration::from_secs(15);

        let version = loop {
            match tokio::time::timeout(duration, reader.receive()).await {
                Ok(Ok(frame)) => {
                    let (header, payload) = frame;
                    let code = header.code;
                    match code {
                ...
                Ok(Err(err)) => {
                    warn!("Version fetching error: {:?}", err);
                    break "2.x".to_string();
                }
                Err(err) => {
                    tracing::warn!("Version fetching timeout: {:?}", err);
                    break "2.x".to_string();
                }
            }
        };
        ...
        sender
            .send(OpCode::Text, &sonic_rs::to_vec(&login).unwrap())
            .await
            .map_err(Error::from)?;
        sender.flush().await.map_err(Error::from)?;
        loop {
            if let Ok(frame) = reader.receive().await {
                ...
                match code {
                    OpCode::Text => {
                        ...
                        match data {
                            WsRecvData::Conn => {
                                tracing::trace!("received ping message");
                                ok?;
                                break;
                            }
                            _ => unreachable!(),
                    ...
                    _ => {
                        println!("unexpected frame on login: {:?}", frame);
        ...
        let (ws, msg_recv) = flume::bounded(1024);
        let ws2: flume::Sender<WsMessage<bytes::Bytes>> = ws.clone();
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        if let Err(err) = sender.send(OpCode::Ping, b"TAOS").await {
            ...
            if sender.version.is_support_binary_sql {
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        let now = Instant::now();
                        match sender
                            .send_recv(WsSend::Binary(bytes::Bytes::copy_from_slice(
                                req_vec.as_slice(),
                            )))
                            .await
                        {
