...
use ws_tool::codec::AsyncDeflateRecv;
...
use ws_tool::{
    ...
    errors::WsError as WsErrorWst,
    frame::{OpCode, OwnedFrame},
    ClientBuilder, Message as WsMessage,
};
...
impl WsTaos {
    ...
    pub(crate) async fn from_wsinfo(info: &TaosBuilder) -> RawResult<Self> {
        ...
        let uri = info.to_query_url().try_into().unwrap();
        use futures::Sink;
...
#[tokio::test]
async fn test_client_ws_tool() -> anyhow::Result<()> {
    use futures::TryStreamExt;
    std::env::set_var("RUST_LOG", "debug");
    // pretty_env_logger::init();
    let dsn = std::env::var("TDENGINE_ClOUD_DSN").unwrap_or("http://localhost:6041".to_string());
    ...
    let dsn = dsn.into_dsn()?;
    let info = TaosBuilder::from_dsn(dsn)?;
    let client = WsTaos::from_wsinfo(&info).await?;

    let _version = client.version();
    log::info!("version: {}", _version);
    let r = client.query("select 1").await?;
    log::info!("r: {:?}", r);
    assert_eq!(client.exec("drop database if exists abc_b").await?, 0);
    assert_eq!(client.exec("create database abc_b").await?, 0);
    assert_eq!(
        client
            .exec("create table abc_b.tb1(ts timestamp, v int)")
            .await?,
        0
    );
    assert_eq!(
        client
            .exec("insert into abc_b.tb1 values(1655793421375, 1)")
            .await?,
        1
    );

    // let mut rs = client.s_query("select * from abc_a.tb1").unwrap().unwrap();
    let mut rs = client.query("select * from abc_b.tb1").await?;
    ...
    assert_eq!(client.exec("drop database abc_b").await?, 0);
    Ok(())
}
