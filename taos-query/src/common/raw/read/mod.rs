...
pub trait ReadTaosExt: ReadExactSizeBytesExt + Buf {
    ...
    fn read_field(&mut self) -> Result<Field, std::io::Error> {
        ...
        let ty = Ty::from(ty);
        dbg!(ty);
        let flags = self.read_i8()?;
        let bytes = self.read_i32v()?;
        let col_id = self.read_i16v()?;
...
impl MetaCreateTable {
    pub fn parse(table_cursor: &mut Cursor<&Bytes>) -> std::io::Result<Self> {
        ...
        dbg!(&t);
        let commentLen = table_cursor.read_i32::<LE>()?;
        dbg!(&commentLen);
        let comment = if commentLen > 0 {
            eprintln!("Warning: create table {:?} has comment length {}, which is not supported in raw data.", name, commentLen);
    ...
    pub fn parse(&self) -> std::io::Result<()> {
        ...
        match res_msg_type {
            ...
            TDMT_VND_DELETE => {
                ...
                if let Ok(ctimeMs) = cursor.read_i64::<LE>() {
                    let ts = Timestamp::new(ctimeMs, crate::common::Precision::Millisecond);
                    dbg!(ts);
                }
                if let Ok(source) = cursor.read_i8() {
                    dbg!(source);
